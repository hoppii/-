...
if(!empty($_POST['name']) && !empty($_POST['comment'])){
    try{    
        $sql  = 'INSERT INTO scott (name, comment) VALUES (:NAMAE, :COMMENT)';
        $stmt = $dbh->prepare($sql);
        $params = array(':NAMAE' => $_POST['name'], ':COMMENT' => $_POST['comment']); // 挿入する値を配列に格納する
        $stmt->execute($params); //挿入する値が入った変数をexecuteにセットしてSQLを実行

        header('location: http://localhost:5501/professors_file/3-5-1scott.php');
        exit();
    } catch (PDOException $e) {
...
<div id="description">
<h3>キーワード</h3>
<h4>Communication technology, intercultural communication, gender communication, social networks</h4>
<hr>
<p>
I am interested in how people use technology to communicate,<br> 
comparing use in different countries when possible. <br> 
Past research includes emotional transmission in text messages and the use of communication technologies <br> 
by international exchange students.<br> 
</p>
</div>
</div>
